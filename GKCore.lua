--[[
    ...
    Changelog:
        ...
        Mar 1, 2016:
            [Commit by einsteinK]
            - Fixed error in LogService
            - Improved bad formatting
            - Fixed ManagedSignal dropping trailing nils
]]
...
local GKCore = {} do
	...
	local LogService = {} do
		...
		RBX_LogService["MessageOut"]:connect(function(Message, MessageType)
			local typ,msg = Message:match("^%[GameKeeperCore (%w+)%] (.*)$")
			if  System["Logs"][typ] then
				table.insert(System["Logs"][typ], {msg, os.time()+tick()%1})
			else
				if (MessageType == Enum.MessageType.MessageOutput or MessageType == Enum.MessageType.MessageInfo) then
					table.insert(Data["GameLogs"]["Info"], {Message, os.time()+tick()%1})
				elseif (MessageType == Enum.MessageType.MessageWarning) then
					table.insert(Data["GameLogs"]["Warning"], {Message, os.time()+tick()%1})
				elseif (MessageType == Enum.MessageError) then
					table.insert(Data["GameLogs"]["Error"], {Message, os.time()+tick()%1})
